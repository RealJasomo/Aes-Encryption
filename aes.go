...
var round_constants = [10]byte{0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36}
...
func encrypt_tbox(blocks []byte, keys [][]byte) []byte {
	...
	for i := 1; i < 10; i++ {
		t0 = s_keys[4*i+0] ^ te0[uint8(s0>>24)] ^ te1[uint8(s1>>16)] ^ te2[uint8(s2>>8)] ^ te3[uint8(s3)]
		t1 = s_keys[4*i+1] ^ te0[uint8(s1>>24)] ^ te1[uint8(s2>>16)] ^ te2[uint8(s3>>8)] ^ te3[uint8(s0)]
		t2 = s_keys[4*i+2] ^ te0[uint8(s2>>24)] ^ te1[uint8(s3>>16)] ^ te2[uint8(s0>>8)] ^ te3[uint8(s1)]
		t3 = s_keys[4*i+3] ^ te0[uint8(s3>>24)] ^ te1[uint8(s0>>16)] ^ te2[uint8(s1>>8)] ^ te3[uint8(s2)]
	...
	s2 ^= s_keys[42]
	s3 ^= s_keys[43]
	return linearize(from_u32_block([]uint32{s0, s1, s2, s3}))
}

func from_u32_block(a []uint32) [][]byte {
	b := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		b[i] = make([]byte, 4)
		binary.BigEndian.PutUint32(b[i], a[i])
	}
	return b
}
...
func decrypt_tbox(blocks []byte, keys [][]byte) []byte {
	...
	var t0, t1, t2, t3 uint32
	for i := 1; i < 10; i++ {
		t0 = s_keys[4*i+0] ^ td0[uint8(s0>>24)] ^ td1[uint8(s3>>16)] ^ td2[uint8(s2>>8)] ^ td3[uint8(s1)]
		t1 = s_keys[4*i+1] ^ td0[uint8(s1>>24)] ^ td1[uint8(s0>>16)] ^ td2[uint8(s3>>8)] ^ td3[uint8(s2)]
		t2 = s_keys[4*i+2] ^ td0[uint8(s2>>24)] ^ td1[uint8(s1>>16)] ^ td2[uint8(s0>>8)] ^ td3[uint8(s3)]
		t3 = s_keys[4*i+3] ^ td0[uint8(s3>>24)] ^ td1[uint8(s2>>16)] ^ td2[uint8(s1>>8)] ^ td3[uint8(s0)]
		s0, s1, s2, s3 = t0, t1, t2, t3
	}
	s0 = uint32(inverse_sbox[t0>>24])<<24 | uint32(inverse_sbox[t3>>16&0xff])<<16 | uint32(inverse_sbox[t2>>8&0xff])<<8 | uint32(inverse_sbox[t1&0xff])
	s1 = uint32(inverse_sbox[t1>>24])<<24 | uint32(inverse_sbox[t0>>16&0xff])<<16 | uint32(inverse_sbox[t3>>8&0xff])<<8 | uint32(inverse_sbox[t2&0xff])
	s2 = uint32(inverse_sbox[t2>>24])<<24 | uint32(inverse_sbox[t1>>16&0xff])<<16 | uint32(inverse_sbox[t0>>8&0xff])<<8 | uint32(inverse_sbox[t3&0xff])
	s3 = uint32(inverse_sbox[t3>>24])<<24 | uint32(inverse_sbox[t2>>16&0xff])<<16 | uint32(inverse_sbox[t1>>8&0xff])<<8 | uint32(inverse_sbox[t0&0xff])
	s0 ^= s_keys[40]
	s1 ^= s_keys[41]
	s2 ^= s_keys[42]
	s3 ^= s_keys[43]
